...
import { fetchPoems, type GameWord, type Poem } from "@/lib/store/gameSlice";
...
import {
  ...
  type GuestGameStats,
} from "@/lib/services/guestProgressService";
...
type WordClassStyle = { color: string; letter: string };

export type GameMode = "authenticated" | "guest";
...
const DEFAULT_WORD_CLASS_STYLE_MAP: Record<string, WordClassStyle> = {
  nom: { color: "bg-blue-200", letter: "N" },
  adjectif: { color: "bg-purple-200", letter: "A" },
  verbe: { color: "bg-green-200", letter: "V" },
  adverbe: { color: "bg-pink-200", letter: "D" },
  pronom: { color: "bg-yellow-200", letter: "P" },
  déterminant: { color: "bg-orange-200", letter: "D" },
  préposition: { color: "bg-teal-200", letter: "R" },
  conjonction: { color: "bg-indigo-200", letter: "C" },
  interjection: { color: "bg-red-200", letter: "I" },
  article: { color: "bg-slate-200", letter: "A" },
};

export default function GameExperience({ mode }: GameExperienceProps) {
  const { user } = useUser();
  ...
  const [guestStats, setGuestStats] = useState<GuestGameStats | null>(null);
  const [guestLoading, setGuestLoading] = useState<boolean>(false);

  const isGuest = mode === "guest";
  const wordClassStyleMap = useMemo(() => DEFAULT_WORD_CLASS_STYLE_MAP, []);
  const isLoadingPoems = isGuest ? guestLoading : loading;
  ...
  useEffect(() => {
    ...
    const loadGuestPoems = async () => {
      setGuestLoading(true);
      try {
        const testParam = searchParams.get("test");
        if (typeof window !== "undefined" && testParam === "true") {
          const testPoemData = window.localStorage.getItem("alphi-test-poem");
          if (testPoemData) {
            const testPoem = JSON.parse(testPoemData);
            setAvailablePoems([testPoem]);
            window.localStorage.removeItem("alphi-test-poem");
            return;
          }
        }

        const publicPoems = await PoemService.fetchPoemsPublic();
        setAvailablePoems(publicPoems);
      } catch (error) {
        console.error("Erreur lors du chargement des jeux publics:", error);
        toast.error("Impossible de charger les jeux pour le moment");
        setAvailablePoems([]);
      } finally {
  ...
  useEffect(() => {
    ...
    setGuestStats(GuestProgressService.getStats());
  }, [isGuest]);
  ...
  useEffect(() => {
    const loadWordClasses = async () => {
      ...
      if (isGuest) {
        const classesFromPoem =
          selectedPoem.wordClasses && selectedPoem.wordClasses.length > 0
            ? selectedPoem.wordClasses
            : Array.from(
                new Set(
                  selectedPoem.words
                    .map((word) => word.class)
        ...
        setWordClasses(buildWordClassesFromStrings(classesFromPoem));
    ...
    loadWordClasses();
  }, [isGuest, selectedPoem, session, wordClassStyleMap]);
  ...
  const saveGameProgress = async (finalScore: number) => {
    ...
    if (isGuest) {
      GuestProgressService.saveRound({
        poemId: selectedPoem.id.toString(),
        ...
        <div className="mb-6 rounded-3xl bg-white p-6 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                {isGuest
                  ? "Essayez Alphi sans créer de compte. Vos progrès restent sur cet appareil."
                  : `Bonjour ${user?.firstName ?? user?.username ?? "!"} ! Prêt(e) à jouer ?`}
              </p>
              {isGuest && guestStats && (
                <div className="mt-3 flex flex-wrap gap-3 text-sm text-gray-500">
                  <span>Parties jouées : {guestStats.totalGamesPlayed}</span>
                  <span>Meilleur score : {guestStats.bestScore}</span>
                  <span>
                    Temps cumulé : {Math.round(guestStats.totalTimePlayed)}s
                  </span>
                </div>
              )}
            </div>
